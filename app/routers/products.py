...
from sqlalchemy import select, update, join, func, desc
...
def remove_product_image(url: Optional[str]) -> None:
    """
...
async def get_all_products(
    ...
    in_stock: Optional[bool] = Query(None, description="true — только товары в наличии, false — только без остатка"),
    seller_id: Optional[int] = Query(None, description="ID продавца для фильтрации"),
...
async def get_products_by_category(category_id: int, db: AsyncSession = Depends(get_async_db)):
    ...
    """
    # Проверка на наличие caregory_id
...
async def get_review_by_product(product_id: int, db: AsyncSession = Depends(get_async_db)):
    ...
    result_products = await db.scalars(
        select(ProductModel).where(ProductModel.id == product_id,
                                   #ProductModel.
                                   ProductModel.is_active == True))
    product = result_products.first()
    if not product:
        raise HTTPException(status_code=404,
                            detail="Product not found or inactive")

    # Получаем активные товары в категории
    result_reviews = await db.scalars(
        select(ReviewModel).where(ReviewModel.product_id == product_id,
                                   ReviewModel.is_active == True))
    return result_reviews.all()
