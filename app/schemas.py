...
class Product(BaseModel):
    ...
    rating: Optional[int] = Field(None, description="Рейтинг продукта")
...
class ReviewCreate(BaseModel):
    product_id: int = Field(description="id в таблице Product")
    comment: Optional[str] = Field(None, min_length=2, description="Отзыв на продукт")
    grade: int = Field(gt=0, le=5, description="оценка")
...
class OrderItem(BaseModel):
    ...
    product: Optional[Product] = Field(None, description="Полная информация о товаре")
